...
export default function AdminDashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [studentRes, appRes] = await Promise.all([
          axios.get("https://davv-backend-api.onrender.com/api/users/all-students", config),
          axios.get("https://davv-backend-api.onrender.com/api/jobs/applications", config)
        ]);
  ...
  const handleVerify = async (studentId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(`https://davv-backend-api.onrender.com/api/users/verify/${studentId}`, {}, {
  ...
  const handlePostJob = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post("https://davv-backend-api.onrender.com/api/jobs/post", jobData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleInviteHR = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post("https://davv-backend-api.onrender.com/api/users/invite-hr", 
        { companyName, email }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleUpdateStatus = async (appId, newStatus) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(`https://davv-backend-api.onrender.com/api/jobs/applications/${appId}`, 
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
        ...
        <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[2.5rem] shadow-2xl relative p-8 md:p-12">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start mb-10 border-b border-slate-100 pb-10">
            <div className="w-32 h-32 bg-blue-50 rounded-3xl flex items-center justify-center overflow-hidden border-4 border-white shadow-xl">
              {s.profilePic ? <img src={`https://davv-backend-api.onrender.com${s.profilePic}`} className="w-full h-full object-cover" alt="Profile" /> : <Users size={48} className="text-blue-200" />}
            </div>
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={() => s.resume ? window.open(`https://davv-backend-api.onrender.com${s.resume}`, '_blank') : alert("No Resume")} className="px-5 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase flex items-center gap-2"><FileText size={14}/> Resume</button>
                <button onClick={() => s.marksheetPG ? window.open(`https://davv-backend-api.onrender.com${s.marksheetPG}`, '_blank') : alert("No Marksheet")} className="px-5 py-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase flex items-center gap-2"><Eye size={14}/> Marksheet</button>
