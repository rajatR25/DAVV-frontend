...
export default function AdminDashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [studentRes, appRes] = await Promise.all([
          axios.get(
            "https://davv-backend-api.onrender.com/api/users/all-students",
            config,
          ),
          axios.get(
  ...
  const handleVerify = async (studentId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `https://davv-backend-api.onrender.com/api/users/verify/${studentId}`,
        {},
        {
      ...
      setStudents((prev) =>
        prev.map((s) => (s._id === studentId ? { ...s, isVerified: true } : s)),
      );
  ...
  const handleUpdateStatus = async (appId, newStatus) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `https://davv-backend-api.onrender.com/api/jobs/applications/${appId}`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const pendingVerifications = students.filter(
    (s) => s.isProfileComplete && !s.isVerified,
  );
        ...
        <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[2.5rem] shadow-2xl relative p-8 md:p-12">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start mb-10 border-b border-slate-100 pb-10">
            <div className="w-32 h-32 bg-blue-50 rounded-3xl flex items-center justify-center overflow-hidden border-4 border-white shadow-xl">
              {s.profilePic ? (
                <img
                  src={`https://davv-backend-api.onrender.com${s.profilePic}`}
              ...
              <div className="flex flex-wrap gap-3">
                <button
                  onClick={() =>
                    s.resume
                      ? window.open(
                          `https://davv-backend-api.onrender.com${s.resume}`,
                          "_blank",
                        )
                      : alert("No Resume")
                  }
                  className="px-5 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase flex items-center gap-2"
                >
                ...
                <button
                  onClick={() =>
                    s.marksheetPG
                      ? window.open(
                          `https://davv-backend-api.onrender.com${s.marksheetPG}`,
                          "_blank",
                        )
                      : alert("No Marksheet")
                  }
                  className="px-5 py-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase flex items-center gap-2"
                >
                  <Eye size={14} /> Marksheet
                </button>
                {!s.isVerified && (
                  <button
                    onClick={() => handleVerify(s._id)}
                    className="px-5 py-2 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase"
                  >
                    Verify Profile
                  </button>
                )}
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-sm">
        <h3 className="text-lg font-black mb-6 flex items-center gap-2">
          <Clock className="text-orange-400" /> Verification Queue
        </h3>
        {pendingVerifications.map((student) => (
          ...
          >
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center font-black uppercase text-slate-400">
  ...
  const renderStudentList = () => (
    <div className="bg-white p-8 rounded-[2rem] shadow-sm overflow-x-auto">
      <h3 className="text-xl font-black mb-8 flex items-center gap-3">
        <Users className="text-blue-600" /> Student Directory
      </h3>
      <table className="w-full text-left">
        <thead>
          <tr className="text-slate-400 uppercase text-[10px] font-black border-b border-slate-100">
            ...
            <th className="pb-4 text-center">Profile</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-50">
          {students.map((s) => (
            <tr key={s._id} className="hover:bg-slate-50 transition-colors">
              ...
              <td className="py-5">
                <span
                  className={`px-3 py-1 rounded text-[10px] font-black uppercase ${s.isVerified ? "bg-green-50 text-green-600" : "bg-orange-50 text-orange-500"}`}
                >
    ...
    <div className="flex h-screen bg-[#f4f7f9] overflow-hidden">
      <aside className="w-[280px] bg-white border-r border-slate-100 flex flex-col p-8 flex-shrink-0">
        <div className="flex items-center gap-3 mb-10">
          ...
          <h1 className="font-black text-2xl">TPO ADMIN</h1>
        </div>
        <nav className="flex-1 space-y-2">
          <button
            onClick={() => setActiveTab("overview")}
            className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-bold text-sm ${activeTab === "overview" ? "bg-blue-50 text-blue-600" : "text-slate-400"}`}
          >
            <LayoutDashboard size={20} /> Overview
          </button>
          <button
            onClick={() => setActiveTab("student-list")}
            className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-bold text-sm ${activeTab === "student-list" ? "bg-blue-50 text-blue-600" : "text-slate-500"}`}
          >
            <Users size={20} /> Students
          </button>
          <button
            onClick={() => setActiveTab("post-job")}
            className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-bold text-sm ${activeTab === "post-job" ? "bg-blue-50 text-blue-600" : "text-slate-500"}`}
          >
            <PlusCircle size={20} /> Post Job
          </button>
          <button
            onClick={() => setActiveTab("invite-hr")}
            className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-bold text-sm ${activeTab === "invite-hr" ? "bg-blue-50 text-blue-600" : "text-slate-500"}`}
          >
            <Send size={20} /> Invite HR
          </button>
          <button
            onClick={() => setActiveTab("applications")}
            className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-bold text-sm ${activeTab === "applications" ? "bg-blue-50 text-blue-600" : "text-slate-500"}`}
          >
            <Activity size={20} /> Applications
          </button>
        </nav>
        <button
          onClick={handleLogout}
          className="flex items-center gap-3 px-5 py-4 w-full text-red-500 font-bold"
        >
      ...
      <main className="flex-1 overflow-y-auto p-10 bg-[#f8fafd]">
        <header className="flex justify-between items-center mb-10">
          <span className="bg-blue-100 text-blue-600 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest">
            <Shield size={12} className="inline mr-2" /> Master Access
          </span>
          <div className="w-10 h-10 bg-[#1d3c93] text-white rounded-full flex items-center justify-center font-black">
            A
          </div>
        </header>
        ...
        {activeTab === "post-job" && (
          <div className="bg-white p-10 rounded-[2.5rem] shadow-sm max-w-4xl mx-auto">
            <h3 className="text-2xl font-black mb-8 flex items-center gap-3">
              <PlusCircle className="text-blue-600" /> New Placement Drive
            </h3>
            <form
            ...
            >
              ...
              <textarea
                ...
                rows="3"
                className="md:col-span-2 p-4 bg-slate-50 rounded-2xl font-bold text-sm outline-none"
              ></textarea>
              <input
                name="minCGPA"
                type="number"
              ...
              <button
                type="submit"
                className="md:col-span-2 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase"
              >
                Publish Now
              </button>
            </form>
        ...
        {activeTab === "invite-hr" && (
          <div className="bg-white p-10 rounded-[2.5rem] shadow-sm max-w-xl mx-auto">
            <h3 className="text-2xl font-black mb-8 flex items-center gap-3">
              <Send className="text-blue-600" /> Invite HRs
            </h3>
        ...
        {activeTab === "applications" && (
          <div className="bg-white p-8 rounded-[2rem] shadow-sm overflow-x-auto">
            <h3 className="text-xl font-black mb-8 flex items-center gap-3">
              <Activity className="text-blue-600" /> Applications
            </h3>
            <table className="w-full text-left">
              <thead>
                <tr className="text-slate-400 uppercase text-[10px] font-black border-b border-slate-100">
                  ...
                  <th className="pb-4">Status</th>
                  <th className="pb-4 text-center">Decision</th>
                </tr>
              </thead>
              <tbody>
                {applications.map((app) => (
                  <tr key={app._id} className="hover:bg-slate-50">
                    ...
                    <td className="py-5 font-bold text-sm text-blue-600">
                      {app.job?.companyName}{" "}
                      <p className="text-[10px] text-slate-400">
                        {app.job?.role}
                      </p>
                    ...
                    <td className="py-5">
                      <span
                        className={`px-2 py-1 rounded text-[10px] font-black uppercase ${app.status === "Selected" ? "bg-green-50 text-green-600" : "bg-orange-50 text-orange-500"}`}
                      >
                        {app.status}
                      </span>
                    </td>
